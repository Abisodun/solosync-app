...
import { X, Download, Send, Printer, Mail } from 'lucide-react'; // Added Mail
import { format } from 'date-fns';
import { motion } from 'framer-motion';
import EmailComposerModal from '../communication/EmailComposerModal'; // Added EmailComposerModal import
...
export default function InvoiceView({ invoice, user, onClose }) {
  const [showEmailModal, setShowEmailModal] = useState(false); // Added state
        ...
        <html>
          <head>
            ...
            <style>
              ...
              /* Ensure the badge is visible but not part of print layout if needed */
              .invoice-badge { display: inline-block; margin-left: 10px; }
            </style>
          </head>
          <body>
  ...
  return (
    <>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 overflow-y-auto" // Updated classname
        onClick={onClose}
      >
        <motion.div
          initial={{ scale: 0.95 }} // Updated scale
          animate={{ scale: 1 }}    // Updated scale
          exit={{ scale: 0.95 }}    // Updated scale
          onClick={(e) => e.stopPropagation()}
          className="w-full max-w-4xl my-8"
        >
          <Card className="p-8 rounded-[20px]" style={{ background: 'white' }}> {/* Updated style */}
            {/* Header Actions - Replaced with new structure */}
            <div className="flex items-center justify-between mb-8 print:hidden">
              ...
              <div className="flex items-center gap-3">
                <Button
                  ...
                  className="rounded-[12px] border-blue-300 text-blue-600 hover:bg-blue-50"
                >
            ...
            <div id="invoice-content">
              ...
              <div className="flex justify-between items-start mb-12">
                <div>
                  ...
                  {/* Moved Badge here to preserve its functionality */}
                  <Badge className={`${config.color} text-sm mt-2 mb-4`}>
                    {config.label}
                  </Badge>
      ...
      <EmailComposerModal
        ...
        defaultRecipients={invoice.client_email ? [invoice.client_email] : []}
        defaultSubject={`Invoice ${invoice.invoice_number} from ${user?.full_name || 'Your Business'}`}
        defaultBody={`Hi ${invoice.client_name},\n\nPlease find attached your invoice ${invoice.invoice_number} for ${getCurrencySymbol(user?.currency)}${invoice.amount?.toLocaleString()}.\n\nDue date: ${format(new Date(invoice.due_date), 'MMMM d, yyyy')}\n\nThank you for your business!\n\nBest regards,\n${user?.full_name || 'Your Business'}`}
        entityId={invoice.id}
        entityType="Invoice"
      />
    </>
